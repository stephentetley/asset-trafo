...
module CompilerMonad =
    ...
    /// Note - EquipmentIndex is not as straightforward as it is  
    /// represented here - it is generated by the Master system
    /// not us which implies we cannot generate a hierarchy plus 
    /// equipment (with classes and values) in one go.
    type NameSupply = 
        { EquipmentIndex : int
          FileIndex : int
        }
