...
module CompilerMonad =
    ...
    type CompilerState = 
        { FileIndices : Map<Level, int>
          TemplateState : TemplateState
        }


    let private compilerStateZero : CompilerState = 
        { FileIndices = Map.empty 
          TemplateState = templateStateZero }
    ...
    let inline private apply1 (ma : CompilerMonad<'a>) 
                                (env : CompilerEnv) 
                                (st : CompilerState) : Result<'a * CompilerState, ErrMsg> = 
    ...
    let mapM (mf: 'a -> CompilerMonad<'b>) 
        ...
        CompilerMonad <| fun env state -> 
            let rec work (xs : 'a list)   
                         (st : CompilerState)
                         (fk : ErrMsg -> Result<'b list * CompilerState, ErrMsg>) 
                         (sk : 'b list -> CompilerState -> Result<'b list * CompilerState, ErrMsg>) = 
                match xs with
                | [] -> sk [] st
                | x :: rest -> 
    ...
    let mapMz (mf: 'a -> CompilerMonad<'b>) 
        ...
        CompilerMonad <| fun env state -> 
            let rec work (xs : 'a list)
                         (st : CompilerState)
                         (fk : ErrMsg -> Result<unit * CompilerState, ErrMsg>) 
                         (sk : CompilerState -> Result<unit * CompilerState, ErrMsg>) = 
                match xs with
                | [] -> sk st
                | x :: rest ->
                    match apply1 (mf x) env st with
    ...
    let foldM (action : 'acc -> 'a -> CompilerMonad<'acc>) 
        ...
        CompilerMonad <| fun env state -> 
            let rec work (acc : 'acc) 
                            (xs : 'a list) 
                            (st : CompilerState)
                            (fk : ErrMsg -> Result<'acc * CompilerState, ErrMsg>) 
                            (sk : 'acc -> CompilerState -> Result<'acc * CompilerState, ErrMsg>) = 
                match xs with
                | [] -> sk acc st
                | x1 :: rest -> 
    ...
    let smapM (action : 'a -> CompilerMonad<'b>) 
        ...
        CompilerMonad <| fun env state ->
            ...
            let rec work (st : CompilerState) 
                         (fk : ErrMsg -> Result<seq<'b> * CompilerState, ErrMsg>) 
                         (sk : seq<'b> -> CompilerState -> Result<seq<'b> * CompilerState, ErrMsg>) = 
    ...
    let filterM (mf: 'a -> CompilerMonad<bool>) 
        ...
        CompilerMonad <| fun env state -> 
            let rec work (xs : 'a list)
                         (st : CompilerState)
                         (fk : ErrMsg -> Result<'a list * CompilerState, ErrMsg>) 
                         (sk : 'a list -> CompilerState -> Result<'a list * CompilerState, ErrMsg>) = 
                match xs with
                | [] -> sk [] st
                | y :: ys -> 
